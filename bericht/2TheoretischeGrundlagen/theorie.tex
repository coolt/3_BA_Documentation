...
Während der Schlafenszeit sind alle Periphierien abgeschalten und im Prozessor (hier als Bsp. Cortex M3) wird nur die Konfigurationen im Flash regelmässig "refreshed". Dies ist in der Graphik \ref{sleep_Grundprinzip} an den grünen Spannungsspitzen zu sehen. Ohne "refreshen" des Speichers, gehen die Konfigurationen verloren und vor jeder Aktion muss das System neu komplett initialisert werden.\\
...
Das Schlafen der Applikation tönt gut. Doch wie wacht sie wieder auf? Dies ist ein nicht-triviales Problem, dass sich durch Interrupts lösen lässt.\\

In diesem Unterkapitel wird das Konzept der Interrupt Driven Applikation erklärt (wie auch ????)
